...
function getPointGen() {
  ...
  if(player.FO.points.gte(1))gain = gain.times(layers.FO.formulas.data[0].f(player.points))
...
var displayThings = [
	function(){	
		let s=""
		if (isEndgame())  s+= "Congratulations, you reached the endgame!<br><span style='color:red'>Please don't go past the endgame. It hasn't been tested yet.</span>"
    else if (player.FO.best.gte(1)) s+= "You have 0 layers left to unlock."
		else if (player.X.best.gte(1))  s+= "You have 1 layer left to unlock.<br> Next layer at 1 ??? and 17555 factors."					
		else if (player.S.best.gte(1))  s+= "You have 2 layers left to unlock.<br> Next layer at '1.798F308' upgrade (Just a upgrade name)."
